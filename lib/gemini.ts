...
async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      // Check if error is retryable (503, 429, timeout, etc.)
      ...
      console.log(`Retry attempt ${attempt + 1}/${maxRetries} after ${delayMs}ms due to: ${lastError.message}`)
...
export interface GoalsParseResult {
  goals: ParsedGoal[]
}
...
- If attendance for a member is unclear, don't include it, then mention it in notes
...
export async function parseWeeklyGoals(postContent: string): Promise<GoalsParseResult> {
  return retryWithBackoff(async () => {
    try {
      const genAI = await getGenAI()
      const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' })

      const prompt = `You are analyzing a weekly goals post for an FRC robotics team. 
Extract all goals/tasks/objectives from the following post content and return them as a standardized JSON array.

Your task is to:
1. Identify all goals, tasks, or objectives mentioned in the post
2. For each goal, extract a clear, concise title
3. If there's additional context or details, include it as a description
...
Important:
- Extract all goals, tasks, or action items you can identify
- Keep titles concise (under 100 characters if possible)
- Only include description if there's meaningful additional context
- Return valid JSON only, no markdown formatting
- If no goals are found, return an empty goals array
- Preserve the order of goals as they appear in the post
...
${postContent}`
      ...
      // Parse the JSON
      let parsed: GoalsParseResult
      try {
        parsed = JSON.parse(jsonText) as GoalsParseResult
      } catch (parseError) {
        console.error('Failed to parse JSON from Gemini response:', jsonText)
        throw new Error(`Failed to parse JSON response: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`)
      }
      ...
      if (!Array.isArray(parsed.goals)) {
        throw new Error('Invalid response format: goals must be an array')
      }
      ...
      const validGoals = parsed.goals.filter(
        ...
        description: goal.description && typeof goal.description === 'string' ? goal.description.trim() : undefined
      }))

      return { goals: validGoals }
    } catch (error) {
      console.error('Error parsing weekly goals with Gemini:', error)
      
      if (error instanceof Error) {
        const errorMsg = error.message.toLowerCase()
        
        if (errorMsg.includes('api key') || errorMsg.includes('unauthorized') || errorMsg.includes('403')) {
          throw new Error('Invalid GEMINI_API_KEY. Please verify your API key is correct.')
        }
        
        throw new Error(`Failed to parse goals: ${error.message}`)
      }
      
      throw new Error('Failed to parse goals: Unknown error occurred')
    }
  }, 3, 1000)
}
